...
function openPopupUrl(val)
{
	if(val=="C")
	{
		 return  "privilegeGroupCode.create";
	}
}
function openPopup(val)
{
	...
	$.post(openPopupUrl(val), { data: "John"})
	  .done(function( data ) {
		  closePopup('progress');
		  $('#modelHeader').html("Create Privilege Group");
		  $('#modelBody').html(data);
		  var label="'Sending'"
		  $('#modalFooter').html(' <button id="saveBtn" name="saveBtn" onClick="savePrivilegeGroupCode();this.disabled=true;" class="btn btn-primary" >Save</button>');
...
function toggleIcon()
{
 ...
 function requestCall(modelResponseId)
 {
	  progressPopUp();
	 var status="";
	 alert($('#ids_'+modelResponseId).val());
	 ...
	 $.ajax({
		  	url: 'privilegeGroupCode.statusUpdate', 
		 	type: 'POST',
		 	dataType: 'json',
		  
		    cache: false,
		    data: "datas="+'{ "id": "'+$('#ids_'+modelResponseId).val()+'", "key": "'+Math.floor(Math.random() * 100)+'", "status":  "'+status+'" } ',
		    processData: false,
 ...
 function clearBtn()
 {
	 $('#searchprivilegeGroupCodeCode').val("");
	 $('#searchprivilegeGroupCodeName').val("");
	 $('#fromDate').val("");
	 $('#toDate').val("");
	 filterToggle=false;
	  ...
	  if(validateDate($('#toDate').val(),'To Date ') && validateDate($('#fromDate').val(),'From date ') )
			 {
			 	...
			 	datas='datas={ "privilegeGroupCode": "'+$('#searchPrivilegeGroupCode').val()+'", "status":  "'+status+'", "privilegeGroupCodeName":  "'+$('#searchPrivilegeGroupCodeName').val()+'","currentPage":  "'+$('#navigatePage').val()+'", "fromDate":  "'+$('#fromDate').val()+'","toDate":"'+$('#toDate').val()+'" } ',
		   
			 	$.ajax({
		  	url: '/privilegeGroupCode.searchView', 
		 	type: 'POST',
		 	dataType: 'json',
...
function editPrivilegeGroupCode(id)
{
	 
progressPopUp();
	 
	
	$.post("privilegeGroupCode.edit", { privilegeId: $('#ids_'+id).val()})
	  .done(function( data ) {
		  closePopup('progress');
		  $('#modelHeader').html("Edit Privilege");
		  $('#modelBody').html(data);
		  var label="'Sending'"
		  $('#modalFooter').html(' <button id="saveBtn" name="saveBtn" onClick="updatePrivilegeGroupCode();this.disabled=true;" class="btn btn-primary" >Save</button>');
		  $('#popUp').modal({backdrop: 'static', keyboard: false}) ;
	  }).fail(function() {
		  closePopup('progress');
	  });
...
function startup(data)
{
 ...
 if(jsonObj.obj.length>0)
 {
	for(i=0;i<jsonObj.obj.length;i++)
	{
		...
		"<input name='ids' id='ids_"+id+"' type='hidden' value='"+jsonObj.obj[i].privilegeGroupCodeId+"' />" +
		"</div> <div class='col-sm-6'>"+id+"</div> </div></td>"+
		" <td class='col-xs-2 table-row-height'>"+jsonObj.obj[i].privilegeGroupCodeName+"</td>"+
		"<td class='col-xs-3 table-row-height'>"+jsonObj.obj[i].privilegeGroupCode+"</td>"+
		...
		html+=" > <span class='slider round'></span></label>  " ;
		 ...
		 html+=	"&nbsp; <a href='javascript:editPrivilegeGroupCode("+id+");'><span  title='Edit Privilege Group Details' data-toggle='tooltip' class='fa-stack hover-change' style='vertical-align: top;'>  " +
			"<i class='far fa-circle fa-stack-2x'></i> <i class='fas fa-pencil-alt fa-stack-1x'></i> " +
			"</span></a>" ;
		
		 html+=" &nbsp; <a href='javascript:viewprivilegeGroupCode("+id+");'>" +
			"<span class='fa-stack hover-change'   title='View Privilege Group Details' data-toggle='tooltip'  style='vertical-align: middle;'>  " +
		 ...
		 html+=	"&nbsp; <a href='javascript:viewprivilegeGroupCode("+id+");'><span  title='View Privilege Group Details' data-toggle='tooltip' class='fa-stack hover-change' style='vertical-align: top;'>  " +
...
function viewprivilegeGroupCode(id)
{

	
	$.post("privilegeGroupCode.showPrivilegeDetails", { privilegeId: $('#ids_'+id).val()})
	  .done(function( data ) {
		  closePopup('progress');
