...
function selectCheckBox(val) {
	...
	var checkBoxes = $("#selectBox_" + val);

	if (checkBoxes.prop("checked") == true) {
		$("#selectBox_" + val).prop("checked", false);
	} else if (checkBoxes.prop("checked") == false) {
		$("#selectBox_" + val).prop("checked", true);
...
function gotoPage(val) {
	...
	$.ajax({
		...
		error: function()
		{
			$('#dataContent').html('<p>An error has occurred</p>');
			closePopup('progress');
		},
...
{ 
	var errorMessage="";
	 obj.forEach(function(element) {
			...
			$('#errorPopUpmodelBody').html("<ul>"+errorMessage+"</ul>");
	 
			$('#errorPopUpmodelHeader').html('<i style="color:red" class="fa fa-exclamation-triangle" aria-hidden="true"></i> Error');	
	$('#errorPopUp').modal();
}
function successPop(obj)
{ 
	$('#modelSuccessMsg').html(obj);
	 $('#modelSuccess').modal();
}
